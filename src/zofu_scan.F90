...
!------------------------------------------------------------------------
  ...
  subroutine test_module_init(self, filename)
...
!------------------------------------------------------------------------
  ...
  subroutine test_module_write_driver(self, filename, mpi)
    ...
    logical, intent(in), optional :: mpi !! Whether tests are parallelized using MPI
    ! Locals:
    integer :: unit
    logical :: use_mpi
    ...
    write(unit, '(a, a/)') "  ! Driver program for Zofu unit tests in ", &
